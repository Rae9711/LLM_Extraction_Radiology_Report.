...
class PMFTextDataset(Dataset):
	"""Dataset that returns (x, (idx_duration, event)) for PMF training."""
...
def _collate_pmf_batch(batch: List) -> Tuple[Dict[str, torch.Tensor], Tuple[torch.Tensor, torch.Tensor]]:
	"""Collate train batch: (x_batch, (idx_durations, events))."""
	xs, ys = zip(*batch)
	...
	return x_batch, (idx_durations, events)
...
class BertForPMF(PreTrainedModel):
	"""BERT encoder + MLP head outputting logits per discrete time; forward returns (batch, out_features)."""
...
def predict_surv_df_from_loader(model: PMF, eval_loader: DataLoader, device: torch.device) -> pd.DataFrame:
	"""Build survival DataFrame by iterating eval_loader and applying PMF pmf/surv logic."""
	model.net.eval()
	all_phi = []
	with torch.no_grad():
	...
	pmf = pad_col(phi_cat).softmax(1)[:, :-1]
	surv = 1 - pmf.cumsum(1)
	surv_np = surv.numpy()
	index = model.duration_index
...
class TrainingConfig:
	...
	eval_steps: int = 200
	save_steps: int = 200
	output_dir: str = "./outputs_pmf"
	data_path: str = ""


def parse_args():
	parser = argparse.ArgumentParser(description="Train PMF survival model on text data")
	parser.add_argument(
		"--base_model",
		type=str,
	...
	parser.add_argument(
		...
		type=int,
		default=10,
		help="Number of discrete duration intervals for PMF label transform",
	)
...
def main():
	...
	print("=" * 60)
	print(f"PMF Training Configuration for run: {args.run_name}")
	print("=" * 60)
	...
	# PMF label transform (LabTransDiscreteTime): returns (idx_durations, events) only
	labtrans = PMF.label_transform(args.num_durations)
	durations_train = train_df["duration"].values
	print(train_df['duration'].min())
	print(train_df['duration'].max())
	print(eval_df['duration'].min())
	print(eval_df['duration'].max())
	events_train = train_df["event"].values

	idx_durations_train, events_train = labtrans.fit_transform(durations_train, events_train)
	durations_eval = eval_df["duration"].values
	events_eval = eval_df["event"].values
	# Keep original durations_eval, events_eval for EvalSurv (no transform needed for eval labels there)

	train_dataset = PMFTextDataset(
		train_df,
		tokenizer=tokenizer,
	...
	net = BertForPMF(base_config, out_features=out_features).to(device)
	...
	for epoch in range(args.num_epochs):
		...
		for x_batch, (idx_durations, events) in pbar:
			x_batch = {k: v.to(device) for k, v in x_batch.items()}
			...
			optimizer.zero_grad()
			phi = model.net(**x_batch)["output"]
			loss = model.loss(phi, idx_durations, events)
			loss.backward()
		...
		surv_df = predict_surv_df_from_loader(model, eval_loader, device)
